function kappa = ivpl2condchebpw(al,be,ga,asbs,c,m)
%IVPL2CONDCHEBPW   Estimate the absolute condition number of a second-order
%collocation problem on a piecewise-Chebyshev grid.
%
%   KAPPA = IVPL2CONDCHEBPW(AL,BE,GA,ASBS,[ C10 C11 C12; C20 C21 C22 ],M)
%   estimates the absolute condition number when the IVP
%
%     AL(T)*U(T) + BE(T)*U'(T) + GA(T)*U''(T) = G(T),
%     C10*U(A) + C11*U'(A) + C12*U''(A) = D1,
%     C20*U(A) + C21*U'(A) + C22*U''(A) = D2,
%
%   is solved on a piecewise-Chebyshev grid.
%
%   Inputs:
%
...
%     [AJ,BJ] in a partition of the problem domain
%
...
narginchk(6,6);
natecheck('ivpl2condchebpw',al,be,ga,asbs,c,m);
...
gas = samplechebpw(ga,asbs,m);
J1 = [ -ones(m+2,1) eye(m+2) ];
J2 = [ -ones(m+1,1) eye(m+1) ];
K1 = intmatrixcheb(m+1);
K2 = intmatrixcheb(m);
...
for trial = 1:3
  for k = 1:5
    y = Ltinvtimes(als,bes,gas,c,m,n,ls,J1,J2,K1,K2,E1,E2,x);
    xi = nzsign(y);
    z = Linvtimes(als,bes,gas,c,m,n,ls,J1,J2,K1,K2,E1,E2,xi);
    [M,j] = max(abs(z));
    if M<=z'*x
      break;
    end
  ...
  kappaabs(trial) = norm(Ltinvtimes(als,bes,gas,c,m,n,ls,J1,J2,K1,K2,E1,E2,x),1);
...
function y = Linvtimes(als,bes,gas,c,m,n,ls,J1,J2,K1,K2,E1,E2,x)
...
L1 = [ c(1,1)*eye(1,m+3) c(1,2)*eye(1,m+2)    c(1,3)*eye(1,m+1)    ;
       J1                -ls(1)/2*K1(2:end,:) zeros(m+2,m+1)       ;
       c(2,1)*eye(1,m+3) c(2,2)*eye(1,m+2)    c(2,3)*eye(1,m+1)    ;
       zeros(m+1,m+3)    J2                   -ls(1)/2*K2(2:end,:) ;
...
cumsumw = zeros(m+1,1);
for j = 2:n
  Lj = [ eye(m+3)          -ls(j)/2*K1       zeros(m+3,m+1) ;
         zeros(m+2,m+3)    eye(m+2)          -ls(j)/2*K2    ;
         diag(als(:,j))*E1 diag(bes(:,j))*E2 diag(gas(:,j)) ];
  cumsumv = cumsumv+ls(j-1)/2*v(:,j-1);
  cumsumw = cumsumw+ls(j-1)/2*w(:,j-1);
  rhsj = [ e(:,j)+u(1)*ones(m+3,1)+ones(m+3,1)*lambdas1'*cumsumv ;
           f(:,j)+v(1)*ones(m+2,1)+ones(m+2,1)*lambdas2'*cumsumw ;
           g(:,j) ];
  solj = Lj\rhsj;
  u(:,j) = solj(1:m+3);
  v(:,j) = solj(m+4:2*m+5);
...
function y = Ltinvtimes(als,bes,gas,c,m,n,ls,J1,J2,K1,K2,E1,E2,x)
...
for j = n:-1:2
  Lj = [ eye(m+3)       zeros(m+3,m+2) (diag(als(:,j))*E1)' ;
         -ls(j)/2*K1'   eye(m+2)       (diag(bes(:,j))*E2)' ;
         zeros(m+1,m+3) -ls(j)/2*K2'   diag(gas(:,j))       ];
  rhsj = [ u(:,j)                                      ;
           v(:,j)+ls(j)/2*lambdas1*ones(1,m+3)*cumsume ;
           w(:,j)+ls(j)/2*lambdas2*ones(1,m+2)*cumsumf ];
  solj = Lj\rhsj;
  e(:,j) = solj(1:m+3);
  f(:,j) = solj(m+4:2*m+5);
  g(:,j) = solj(2*m+6:end);
...
L1 = [ J1'                   zeros(m+3,m+1)        (diag(als(:,1))*E1)' c(1,1)*eye(m+3,1) c(2,1)*eye(m+3,1) ;
       -ls(1)/2*K1(2:end,:)' J2'                   (diag(bes(:,1))*E2)' c(1,2)*eye(m+2,1) c(2,2)*eye(m+2,1) ;
       zeros(m+1,m+2)        -ls(1)/2*K2(2:end,:)' diag(gas(:,1))       c(1,3)*eye(m+1,1) c(2,3)*eye(m+1,1) ];
rhs1 = [ u(:,1)+sum(sum(e(:,2:n)))*eye(m+3,1)                                      ;
         v(:,1)+ls(1)/2*lambdas1*ones(1,m+3)*cumsume+sum(sum(f(:,2:n)))*eye(m+2,1) ;
         w(:,1)+ls(1)/2*lambdas2*ones(1,m+2)*cumsumf                               ];
sol1 = L1\rhs1;
